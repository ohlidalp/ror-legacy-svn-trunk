...
void MetaVec4::SetDefaults(const CDDObject *pMetaKey, const char *pszMetaID, CDDObject *pData)
{
	if(pData->GetKeyType(pszMetaID)!=DD_VEC4)
	{			
		pData->SetVec4(pszMetaID, pMetaKey->GetVec4("Default"));					
	}			
}

void MetaVec4::OnCreated()
{
...
void MetaVec4::UpdateData(CDDObject *pData)
{	
	...
	_snprintf_s(buffer, 25, "%.3f", m_Value.w);
	m_pEditCtrls[3]->SetWindowText(buffer);
}

void MetaVec4::OnPaint()
{
	PAINTSTRUCT ps;
	...
	if(m_bShowControls)
	{
		...
		rTempLabel.bottom+=g_iDefaultCtrlHeight*4;
		...
		rTempLabel.top+=g_iDefaultCtrlHeight;
		rTempLabel.bottom+=g_iDefaultCtrlHeight;
		::DrawText(hdc, "W", -1, &rTempLabel, DT_SINGLELINE|DT_VCENTER|DT_LEFT);
	}
	else
		::DrawText(hdc, "+", -1, &rLabel, DT_SINGLELINE|DT_VCENTER|DT_LEFT);
	rLabel.left+=10;
	...
	char buffer[50];
	_snprintf(buffer, 50, "{%.3f, %.3f, %.3f, %.3f}", m_Value.x, m_Value.y, m_Value.z, m_Value.w);
...
bool MetaVec4::OnMouseDown(int iFlags, int iX, int iY)
{	
	if(iX<10 && iY<=g_iDefaultCtrlHeight && m_bEnabled)
	{
		...
		if(m_bShowControls)
			m_iHeight=g_iDefaultCtrlHeight*5;
		else
...
void MetaVec4::OnCommand(HWND hWnd, int iCode, int iID)
{
...
void MetaVec4::SetEnabled(bool bEnabled)
{
