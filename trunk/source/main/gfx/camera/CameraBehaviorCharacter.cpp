...
{
	targetDirection = -globalEnvironment->player->getRotation() - Radian(Math::HALF_PI);
	camLookAt       =  globalEnvironment->player->getPosition() + camPositionOffset;
...
bool CameraBehaviorCharacter::mouseMoved(const CameraManager::cameraContext_t &ctx, const OIS::MouseEvent& _arg)
{
	if ( camMode == CHARACTER_FIRST_PERSON )
	{
		...
		Radian angle = globalEnvironment->player->getRotation();
		...
		globalEnvironment->player->setRotation(angle);
