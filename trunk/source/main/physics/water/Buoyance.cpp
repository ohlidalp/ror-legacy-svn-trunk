...
Vector3 Buoyance::computePressureForceSub(Vector3 a, Vector3 b, Vector3 c, Vector3 vel, int type)
{
	...
	if (surf<0.00001) return Vector3::ZERO;
	...
	if (type!=BUOY_DRAGONLY)
	{
		...
		Vector3 ap=a+(globalEnvironment->terrainManager->getWater()->getHeightWaves(a)-a.y)*9810*normal;
		Vector3 bp=b+(globalEnvironment->terrainManager->getWater()->getHeightWaves(b)-b.y)*9810*normal;
		Vector3 cp=c+(globalEnvironment->terrainManager->getWater()->getHeightWaves(c)-c.y)*9810*normal;
		//find centroid
	...
	if (type!=BUOY_DRAGLESS)
	{
		...
		vel=vel-globalEnvironment->terrainManager->getWater()->getVelocity(tc);
	...
	//		drg=(-500.0*surf*vell*cosaoa)*vel;
			...
			if (update && splashp)
			{
				...
				if (fxl>1.5) //if enough pushing drag
				{
					...
					if (globalEnvironment->terrainManager->getWater()->getHeightWaves(a)-a.y<0.1) splashp->malloc(a, fxdir);
					else if (globalEnvironment->terrainManager->getWater()->getHeightWaves(b)-b.y<0.1) splashp->malloc(b, fxdir);
					else if (globalEnvironment->terrainManager->getWater()->getHeightWaves(c)-c.y<0.1) splashp->malloc(c, fxdir);
				}
			}
		}
	}
...
{
	float wha=globalEnvironment->terrainManager->getWater()->getHeightWaves((a+b+c)/3.0);
...
{
	if (a->AbsPosition.y>globalEnvironment->terrainManager->getWater()->getHeightWaves(a->AbsPosition) && b->AbsPosition.y>globalEnvironment->terrainManager->getWater()->getHeightWaves(b->AbsPosition) && c->AbsPosition.y>globalEnvironment->terrainManager->getWater()->getHeightWaves(c->AbsPosition)) return;
	//compute center
