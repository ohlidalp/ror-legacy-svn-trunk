				...
				for (size_t v = 0; v < sourceVertexData->vertexCount; ++v)
				{
					...
					for (ei = elems.begin(); ei != elems.end(); ++ei)
					{
						...
						case VES_DIFFUSE:
							...
							tmpR = (uint8)(((tmpColor) & 0xFF) * queuedMesh.color.r);
              tmpG = (uint8)(((tmpColor >> 8) & 0xFF) * queuedMesh.color.g);
              tmpB = (uint8)(((tmpColor >> 16) & 0xFF) * queuedMesh.color.b);
              tmpA = (uint8)((tmpColor >> 24) & 0xFF);
				...
				//Generate color
        uint8 tmpR = (uint8)(queuedMesh.color.r * 255);
        uint8 tmpG = (uint8)(queuedMesh.color.g * 255);
        uint8 tmpB = (uint8)(queuedMesh.color.b * 255);
        uint32 tmpColor = (uint32)(tmpR | (tmpG << 8) | (tmpB << 16) | (0xFF << 24));
